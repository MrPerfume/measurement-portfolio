...
私有系统是 Laravel/Filament 单体应用；作品集仓库只抽取不依赖框架的领域规则，并用一个纯静态交互原型演示这些规则如何影响页面。这样既能直接运行边界测试，也能让审阅者亲手走一遍业务判断，同时把数据库结构、真实页面路由、外部通知、环境配置和内部运维完全留在私有边界内。
...
`site/` 是展示层，不是私有后台的前端副本：

- `index.template.html` 提供语义结构和无脚本回退信息；
- `data/demo.json` 是唯一演示数据契约，只使用 `DEMO-*` 合成标识；
- `state.mjs` 是纯状态转换函数，覆盖异常隔离、提醒边界、回件和证书复核；
- `app.mjs` 只负责页面渲染与事件委派，不读写浏览器存储；
- `build-pages.php` 在构建时内嵌演示数据，运行时不发起数据请求。

页面由 GitHub Pages 托管，CSP 明确设置 `connect-src 'none'`。没有登录、Cookie、表单提交、数据库、后端 API、分析脚本或第三方资源；交互状态只存在当前页面内存中。
...
公开内核和静态原型都没有伪装成完整系统。被省略的适配器必须在真实项目中另行覆盖授权测试、数据库事务测试、迁移回滚、外部请求伪造和浏览器验收。
